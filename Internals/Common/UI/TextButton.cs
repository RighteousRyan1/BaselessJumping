...
namespace BaselessJumping.Internals.Common.UI
{
    ...
    public class TextButton
    {
        ...
        public TextButton(string text, SpriteFont font,
            Color color, Vector2 drawPos, Action<TextButton> onClick, float scalingAmount = 0.0175f, bool giveBorder = false, Color borderColor = default, SoundEffect soundClick = null, SoundEffect soundHovered = null, 
            bool canDraw = true)
        {
            this.text = text;
            this.font = font;
        ...
        public void Draw(bool beginBatch = true)
        {
            ...
            if (border)
                Utilities.DrawTextWithBorder(font, text, drawPosition, color, borderColor, 0f, scale, 2);
            else
